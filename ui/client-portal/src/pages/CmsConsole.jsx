...
export default function CmsConsole() {
  ...
  async function load() {
    setErr("");
    try {
      const r = await fetch(`${API_BASE}/internal/cms/last`);
  ...
  return (
    <div className="bg-white rounded-2xl shadow p-6">
      ...
      <p className="mt-1 text-sm text-gray-600">
        Worker → API Gateway → CMS SOAP mock (auto)
      </p>
      ...
      <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <Info label="Client ID" value={data?.client_id || "—"} mono />
      </div>

      <div className="mt-4 rounded-2xl border bg-gray-50 p-4">
        <div className="text-sm font-semibold text-gray-700">SOAP Request (XML)</div>
