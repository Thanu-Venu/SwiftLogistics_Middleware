...
export default function App() {
  ...
  // reconnect websocket whenever orderId changes
  useEffect(() => {
    if (!orderId) return;
    connectWs();
    return () => disconnectWs();
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto px-4 py-10">
        <header className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight">
              SwiftLogistics <span className="text-gray-500">Client Portal</span>
            </h1>
          ...
          <div className="bg-white rounded-2xl shadow p-6">
            ...
            <div className="mt-5 space-y-4">
              ...
              <div className="flex items-center gap-2">
                <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${badgeClass(currentStatus)}`}>
              ...
              <div className="flex gap-2">
                <button
                  onClick={fetchStatus}
                  disabled={!orderId}
                  className="flex-1 rounded-xl border py-2 font-semibold hover:bg-gray-50 disabled:opacity-50"
                >
                  Refresh (GET)
                </button>
                <button
              ...
              <p className="text-xs text-gray-500">
                Tip: Worker is very fast right now, so you may see only the final status. Add <span className="font-mono">time.sleep(2)</span> between worker steps to visualize.
              </p>
            </div>
          </div>
        </div>
        ...
        <footer className="mt-10 text-xs text-gray-500">
          Built with FastAPI + RabbitMQ + PostgreSQL + mock CMS(SOAP)/ROS(REST)/WMS(TCP) services.
        </footer>
      </div>
    </div>
  );
